...
class Board(models.Model):
    ...
    description = models.TextField(
        blank=True, help_text=_("Optional description of the board's purpose")
    )
    is_public = models.BooleanField(
        default=True,
        help_text=_(
            "Public boards are visible to all users, private boards require membership"
        ),
    )
...
class Comment(models.Model):
    ...
    content = models.TextField(
        validators=[MinLengthValidator(3)], help_text=_("The comment content")
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
